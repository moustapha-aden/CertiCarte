...
<div class="rounded-xl border-2 {{ $variantClass }} p-6 hover:shadow-lg transition-all duration-300 group cursor-pointer" 
     @if(isset($onclick)) onclick="{{ $onclick }}" @endif>
    {{-- Card Header --}}
    @if($title || $icon)
        <div class="flex items-center space-x-4 mb-4">
            @if($icon)
                <div class="w-12 h-12 bg-indigo-500 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform duration-200">
